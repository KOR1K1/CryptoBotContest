...
> Это backend-система аукционов цифровых товаров, вдохновленная механикой **Telegram Gift Auctions**. Я не пытался угадать или воспроизвести внутреннюю логику Telegram — она все равно не публична. Вместо этого я смотрел на поведение продукта, фиксировал допущения там, где информации нет, и строил систему так, как ее пришлось бы делать в реальном продакшене: с деньгами, конкуренцией и нагрузкой.
Проект про аккуратную финансовую модель, устойчивость к race conditions и архитектуру, которую не стыдно поддерживать.
...
[![TypeScript](https://img.shields.io/badge/TypeScript-4.х-blue?style=for-the-badge&logo=typescript&logoColor=white)](https://github.com/KOR1K1/CryptoBotContest.git)
...
[![MongoDB](https://img.shields.io/badge/MongoDB-4.х-green?style=for-the-badge&logo=mongodb&logoColor=white)](https://github.com/KOR1K1/CryptoBotContest.git)
[![Redis](https://img.shields.io/badge/Redis-6.х-DC382D?style=for-the-badge&logo=redis&logoColor=white)](https://github.com/KOR1K1/CryptoBotContest.git)
...
- **[`SPEC.md`](./docs/SPEC.md)** — детальная спецификация механики аукциона
- **[`ASSUMPTIONS.md`](./docs/ASSUMPTIONS.md)** — все явные допущения
- **[`PERFORMANCE_OPTIMIZATION.md`](./PERFORMANCE_OPTIMIZATION.md)**, **[`PERFORMANCE_REPORT.md`](./PERFORMANCE_REPORT.md)** — оптимизации и масштабируемость
...
Здесь каждая операция — отдельная запись в immutable ledger. Баланс можно восстановить из истории, а инварианты проверяются явно:
...
Бизнес-логика не размазана по контроллерам. Она живет в domain-сервисах: AuctionService, BidService, BalanceService.
Контроллеры делают ровно то, что должны: валидация, авторизация, вызов сервисов.
...
Все цифры получены на реальных нагрузочных тестах. Подробности — в [`PERFORMANCE_REPORT.md`](./PERFORMANCE_REPORT.md)
...
ИИ использовался осознанно: для ускорения генерации кода и поиска документации.
Все решения принимались вручную, каждая часть логики проверялась, все допущения зафиксированы в документации.
...
**Подробнее**: [`SINGLE_SERVER_OPTIMIZATIONS.md`](./docs/SINGLE_SERVER_OPTIMIZATIONS.md)

---

## Итог

Это production-grade backend аукциона с нормальной финансовой моделью, устойчивостью к конкуренции и понятной архитектурой.
Не идеальный. Зато честный, проверяемый и пригодный для реального использования.